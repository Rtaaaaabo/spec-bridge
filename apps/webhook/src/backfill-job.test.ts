...
test("使った額が上限未満なら続ける", () => {
  assert.equal(withinBudget({ done: 3, failed: 0, pending: 2, costUsd: 5.1 }, 10), true);
});

test("上限に達したら書かない", () => {
  assert.equal(withinBudget({ done: 6, failed: 0, pending: 1, costUsd: 10 }, 10), false);
  assert.equal(withinBudget({ done: 7, failed: 0, pending: 1, costUsd: 11.9 }, 10), false);
});

// --- 仕上げの待ち合わせ（ワーカーが複数いると完了順は保証されない） ---

test("残っている機能があるあいだは仕上げない", () => {
  assert.equal(readyToFinish({ done: 2, failed: 0, pending: 1, costUsd: 3 }), false);
});

test("失敗も「終わった」として扱う（1件の失敗で PR を出せなくしない）", () => {
  assert.equal(readyToFinish({ done: 2, failed: 1, pending: 0, costUsd: 3 }), true);
});
...
test("兄弟ジョブから、成功・失敗・残り・費用を数える", async () => {
  ...
  assert.deepEqual(await runProgress(store, "run-1"), {
    done: 1,
    failed: 1,
    pending: 1,
    costUsd: 1.7,

...
2ファイルの PR から、docs リポジトリへ610行の PR が生成されました。
（docs リポジトリは非公開です。生成物には内部のファイルパスと仕様が含まれるため、
そうあるべきものです）
...
あわせて、一覧ページ `README.md` と、全機能の「開発者への確認事項」を1ページに集めた
`open-questions.md` が再生成されます。コードからは確定できず推測で埋めなかった点を、ここで見渡せます。

確認事項は3種類に分けて出します。人に聞くべきなのは先頭の「聞くべきこと」だけです。

| 種類 | 中身 |
| --- | --- |
| 聞くべきこと（`intent`） | コードを探したうえで、意図・運用など人に確かめるしかない点。答えを探して開いたファイルが付く |
| 追加で調べれば埋まる可能性があるもの（`unverified`） | コードを追えば分かるはずで、今回そこまで読めなかった点 |
| ドキュメントの範囲についてのメモ（`scope`） | 別ドキュメントに分けるべきか、などの相談 |

「聞くべきこと」には、エージェントが実際に開いたファイルが最低1つ必要です。
開いていないファイルしか挙げられなかった項目は、機械的に「追加で調べれば埋まる可能性があるもの」に格下げされます。

実行の最後に、所要時間・エージェント呼び出し回数・推定コストを表示します。
推定コストは API 料金換算で、Claude のサブスクリプションで認証している場合は実際の請求額ではありません。
...
### 既存のコードから一式を書き起こす（バックフィル）

PR 単位の解析だけだと、導入した日には何も無い状態から始まります。
**いまのコードから機能ドキュメント一式を作る**のがバックフィルです。

```bash
pnpm backfill --repo ~/dev/acme-backend --docs ~/dev/acme-specs --limit 12
```

まずリポジトリを探索して**ドキュメント化すべき機能を列挙**し、そのそれぞれについて
PR 解析と同じ手順で仕様を書き起こします。既にドキュメントがある機能は新規作成せず既存に紐づけるので、
何度実行しても重複しません。

| オプション | 説明 |
| --- | --- |
| `--limit` | 生成する機能数の上限（既定 20） |
| `--repo-name` | `org/repo`。省略時は `git remote origin` から推測。判別できなければ中断する |
| `--docs-repo` | `org/repo`。生成結果を docs リポジトリへ **PR として提出**する（省略時はローカルに書くだけ） |

`--docs-repo` を付けると、PR 解析と同じ経路で docs リポジトリに PR を出します。
PR に紐づかないので、本文には代わりに**どのコミットのコードから起こしたか**と、
**列挙 N 件 / 生成 M 件 / 失敗 K 件**・所要時間・推定コストを書きます。
作業ツリーに未コミットの変更がある場合、起点のコミットは書きません（その SHA は起点として正しくないため）。

#### ジョブとして回す（機能ごとに分割）

`pnpm backfill` は手元で全機能を順に処理します。**1機能あたり数分・利用上限に当たりうる**ので、
サーバーで回す場合は機能ごとにジョブを分けます。

```bash
pnpm backfill-run --repo acme/backend --limit 5 --budget 10   # 積む
pnpm worker                                                    # 実行する
```

| | |
| --- | --- |
| `backfill.survey` | 機能を列挙し、機能ごとのジョブを積む（数十秒） |
| `backfill.feature` | 1機能を書き起こし、docs リポジトリのブランチへコミット |
| `backfill.finish` | 一覧と確認事項を作り直し、PR を1本出す |

- 失敗した機能だけを拾い直せます（**1件の失敗で他を巻き込みません**）
- `--budget` を超えたら、残りの機能は書かずに仕上げへ進みます。1機能 約 $1.7 なので歯止めが要ります
- 仕上げは、機能ジョブが全部終わるまで自動で待ちます

**確度の測り方が PR 解析とは違います。** PR があるときは「変更ファイルをどれだけ読んだか」を測れますが、
バックフィルには差分がありません。代わりに**出典に挙げたファイルを実際に開いたかどうか**を測ります。
`file:line` が実在するかの検証（`sourceValidity`）だけでは、
「実在するファイルを、読まずに出典として挙げた」を捕まえられないためです。

変更履歴には PR 参照を入れません。存在しない PR 番号を作らないためです。
...
```
PR マージ → webhook → 署名検証 → ジョブを積む（同じマージは二度積まない）
         → worker が取り出す → 浅くクローン → 分類 → 解析
...
packages/core/          解析パイプラインの中核
  ...
  survey.ts             コードベースから機能を列挙（バックフィル用。差分が無い場合の入口）
...
| Phase | 内容 |
| --- | --- |
| 0（現在） | PR → 機能ドキュメント生成、既存コードからのバックフィル、CLI + サポートデスク画面 |
